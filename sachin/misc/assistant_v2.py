...
# Function to capture and describe a webcam frame without waiting for user click
def capture_webcam_image():
    try:
        ...
        # Optionally display the captured frame briefly (e.g., for 1 second)
        cv2.namedWindow("Webcam Frame")
        cv2.imshow("Webcam Frame", frame)
        cv2.waitKey(1000)  # Display for 1 second, adjust or remove as needed
...
# Modified chat function for continuous session with voice input, skipping empty transcriptions
def chat_with_qwen3():
    messages = []
    print("Starting voice-activated continuous chat session.")
    ...
    try:
        while True:
            ...
            try:
                ...
                if hasattr(response_message, "tool_calls") and response_message.tool_calls:
                    ...
                    response_message = response.choices[0].message
                    print(f"Qwen3 (after tool call): {response_message.content}\n")
                ...
                # Optional: Print conversation history for debugging
        ...
        # Optional: Save conversation history to a file
